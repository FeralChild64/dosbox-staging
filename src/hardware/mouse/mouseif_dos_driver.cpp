...
static bool     rate_locked = false; // true = rate was set by DOS application
...
static void set_interrupt_rate(const uint16_t rate_id)
{
    ...
    if (val_hz) {
        rate_locked = true;
        MouseInterface::GetDOS()->NotifyInterfaceRate(val_hz);
    }
}
...
static void reset_hardware()
{
    ...
    rate_locked = false;
    MOUSEDOS_NotifyMinRate(min_rate_hz, true);
}
                            ...
                            const bool force_update)
{
    ...
    // If we have a minimum rate set by user, stick to it
    uint16_t rate_to_set = min_rate_hz;
