...
static constexpr uint8_t max_sensitivity = 100; // like in INT 0x33 function 0x1A
...
{
    manymouse.NotifyConfigAPI(true);
}

MouseConfigAPI::~MouseConfigAPI()
{
    manymouse.NotifyConfigAPI(false);
}
...
{
    if (sensitivity_x > max_sensitivity ||
        sensitivity_y > max_sensitivity)
        return false;
...
bool MouseConfigAPI::SetSensitivityY(const MouseConfigAPI::ListIDs &list_ids,
                                     const uint8_t sensitivity_y)
{
    if (sensitivity_y > max_sensitivity)
        return false;
