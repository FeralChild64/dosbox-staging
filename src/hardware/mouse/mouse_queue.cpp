...
{
    constexpr long min_ms = 2;   // 500 Hz sampling rate    XXX share rate_hz_min/rate_hz_max with mouse_interfaces.cpp
...
void MouseQueue::SetRateDOS(const uint16_t rate_hz)
{
    ...
    start_delay.dos_ms = ClampStartDelay(1000.0f / rate_hz);
}
...
void MouseQueue::SetRateBUS(const uint16_t rate_hz)
{
    // Convert rate in Hz to delay in milliseconds
    start_delay.bus_ms = ClampStartDelay(1000.0f / rate_hz);
}
...
void MouseQueue::AddEvent(MouseEvent &ev)
{
    ...
    if (!ev.request_dos && !ev.request_ps2 && !ev.request_bus)
    ...
    if (ev.request_ps2 || ev.request_bus) {
        if (ev.request_ps2 && !HasEventPS2() &&
            timer_in_progress && !delay.ps2_ms) {
        ...
        if (ev.request_bus && !HasEventBus() &&
            timer_in_progress && !delay.bus_ms) {
            DEBUG_QUEUE("AddEvent: restart timer for %s", "BUS");
            // Same is true for the Bus/InPort mouse
            restart_timer = true;
        }

        // Events for PS/2 and BUS interfaces (or virtualizer
        // compatible drivers) do not carry any information - they
        // are only  notifications that new data is available
        pending_ps2 |= ev.request_ps2;
        pending_bus |= ev.request_bus;
    }
    ...
    if (restart_timer) {
        ...
        // XXX handle Bus mouse events too
        PIC_ActivateIRQ(12);
    }
}
...
void MouseQueue::StartTimerIfNeeded()
{
    ...
    if (HasEventBus() || delay.bus_ms) {
        timer_needed = true;
        delay_ms     = std::min(delay_ms, delay.bus_ms);
    }
...
void MouseQueue::UpdateDelayCounters()
{
    ...
    delay.bus_ms = calc_new_delay(delay.bus_ms, elapsed);
...
void MouseQueue::Tick()
{
    ...
    // Since BUS mouse has a separate interrupt (no shared INT74,
    // like with DOS and PS/2 BIOS interfaces), it is the easiest
    // to handle it here
    if (HasReadyEventBus()) {
        DEBUG_QUEUE("UpdateData, for %s", "BUS");
        MOUSEBUS_Update();
        delay.bus_ms = start_delay.bus_ms;
        pending_bus = false;
    }
...
bool MouseQueue::HasEventBus() const
{
    return pending_bus;
}
...
{
    return HasEventDos() || HasEventPS2() || HasEventBus();
}
...
bool MouseQueue::HasReadyEventBus() const
{
    return HasEventBus() && !delay.bus_ms;
}
