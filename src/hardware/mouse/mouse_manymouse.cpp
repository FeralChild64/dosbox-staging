...
void ManyMouseGlue::NotifyConfigAPI(const bool startup)
{
    if (startup) {
...
void ManyMouseGlue::Rescan()
{
    ...
    for (uint8_t idx = 0; idx < num_mice; idx++) {
        ...
        physical_devices.push_back(MousePhysical(name));
        mouse_info.physical.push_back(
                                ...
                                const bool critical_only)
{
    ...
    case MANYMOUSE_EVENT_RELMOTION:
        ...
        if (rel_x.size() <= device_idx) {
            rel_x.resize(device_idx + 1, 0);
            rel_y.resize(device_idx + 1, 0);
        }
