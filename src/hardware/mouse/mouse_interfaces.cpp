    ...
    sensitivity_predefined(sensitivity_predefined)
{
    ...
    mouse_info.interfaces.push_back(MouseInterfaceInfoEntry(interface_id));
}

void MouseInterface::Init()
{
    const auto &map_pattern = mouse_config.map_pattern[GetInterfaceIdx()];

    if (map_pattern.empty())
        return;

    // Try to map physical mouse to the interface

    std::regex regex;
    if (!MouseConfigAPI::PatternToRegex(map_pattern, regex)) {
        LOG_WARNING("MOUSE: Invalid map pattern '%s'", map_pattern.c_str());
        return;
    }

    MouseConfigAPI mouse_config_api;
    if (!mouse_config_api.Map(interface_id, regex)) {
        LOG_WARNING("MOUSE: No available mouse matching pattern '%s'", map_pattern.c_str());
        return;
    }
...
{
    return mapped_idx < sizeof(mouse_info.physical);
}
...
{
    ConfigSetSensitivity(mouse_config.sensitivity_x[GetInterfaceIdx()],
                         mouse_config.sensitivity_y[GetInterfaceIdx()]);
}

void MouseInterface::ConfigResetSensitivityX()
{
    ConfigSetSensitivityX(mouse_config.sensitivity_x[GetInterfaceIdx()]);
}

void MouseInterface::ConfigResetSensitivityY()
{
    ConfigSetSensitivityY(mouse_config.sensitivity_y[GetInterfaceIdx()]);
}
...
{
    min_rate_hz = mouse_config.min_rate[GetInterfaceIdx()];
    UpdateRate();
}
...
void MouseInterface::UpdateSensitivity()
{
    auto calculate = [this](const uint8_t user_val)
    {
        ...
        return sensitivity_predefined * std::pow(2.0f, (user_val - 50) / double_steps);
    };
...
{
    rate_hz = interface_rate_hz;

    if (rate_hz) { // 0 is 'N/A'
        rate_hz = std::max(rate_hz, min_rate_hz);
        rate_hz = std::clamp(rate_hz, rate_hz_min, rate_hz_max);
    }
}
