...
// - https://github.com/bochs-emu/Bochs/blob/master/bochs/iodev/busmouse.cc
...
static uint8_t get_value_byte(const io_val_t value)
{
...
static void inp_write_data(io_port_t, io_val_t value, io_width_t)
{
    ...
    else if (state.command == INP_CTRL_COMMAND) {
...
static uint32_t bus_read_control(const io_port_t, const io_width_t)
{
    ...
    state.toggle_counter = (state.toggle_counter + 1) & 0x7FF;
