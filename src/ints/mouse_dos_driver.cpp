...
static struct { // DOS driver state
	...
	uint16_t doubleSpeedThreshold = 0; // TODO: this should affect mouse
	                                   // movement
...
static uint16_t DEFAULT_SCREEN_MASK[Y_CURSOR] = {0x3FFF,
                                                 0x1FFF,
                                                 0x0FFF,
                                                 0x07FF,
                                                 0x03FF,
...
static uint16_t DEFAULT_CURSOR_MASK[Y_CURSOR] = {0x0000,
                                                 0x4000,
                                                 0x6000,
                                                 0x7000,
                                                 0x7800,
...
void DrawCursorText()
{
	...
	if ((GETPOS_Y <= state.updateRegion_y[1]) &&
	    (GETPOS_Y >= state.updateRegion_y[0]) &&
	    (GETPOS_X <= state.updateRegion_x[1]) &&
	    (GETPOS_X >= state.updateRegion_x[0])) {
		return;
	}
...
void MOUSEDOSDRV_DrawCursor()
{
	...
	// Check if cursor in update region
	/*    if ((GETPOS_X >= state.updateRegion_x[0]) && (GETPOS_X <=
	   state.updateRegion_x[1]) && (GETPOS_Y >= state.updateRegion_y[0]) &&
	   (GETPOS_Y <= state.updateRegion_y[1])) { if (CurMode->type==M_TEXT16)
	            RestoreCursorBackgroundText();
	        else
	...
	// Draw Mousecursor
	dataPos               = addy * X_CURSOR;
	const auto screenMask = state.userScreenMask ? state.userDefScreenMask
	                                             : DEFAULT_SCREEN_MASK;
	const auto cursorMask = state.userCursorMask ? state.userDefCursorMask
	                                             : DEFAULT_CURSOR_MASK;
	for (y = y1; y <= y2; y++) {
                             ...
                             const bool is_captured)
{
	...
	if ((fabs(x_rel_sens) > 1.0) || (state.senv_x < 1.0))
		dx *= state.senv_x;
	if ((fabs(y_rel_sens) > 1.0) || (state.senv_y < 1.0))
		dy *= state.senv_y;

	state.mickey_x += (dx * state.mickeysPerPixel_x);
	state.mickey_y += (dy * state.mickeysPerPixel_y);
	if (state.mickey_x >= 32768.0f)
		state.mickey_x -= 65536.0f;
	else if (state.mickey_x <= -32769.0f)
		state.mickey_x += 65536.0f;
	if (state.mickey_y >= 32768.0f)
		state.mickey_y -= 65536.0f;
	else if (state.mickey_y <= -32769.0f)
		state.mickey_y += 65536.0f;
	if (is_captured) {
		state.x += dx;
		state.y += dy;
