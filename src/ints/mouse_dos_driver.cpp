...
void RestoreCursorBackgroundText()
{
    ...
    if (state.background) {
        WriteChar(state.backposx, state.backposy,
                  real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAGE),
                  state.backData[0], state.backData[1], true);
...
void DrawCursorText()
{
    ...
    if ((GETPOS_Y <= state.updateRegion_y[1]) && (GETPOS_Y >= state.updateRegion_y[0]) &&
        (GETPOS_X <= state.updateRegion_x[1]) && (GETPOS_X >= state.updateRegion_x[0])) {
        return;
    }
    ...
    if (state.mode < 2) state.backposx >>= 1; 

    //use current page (CV program)
    uint8_t page = real_readb(BIOSMEM_SEG, BIOSMEM_CURRENT_PAGE);
    
    if (state.cursorType == 0) {
        uint16_t result;
        ...
        // Write Cursor
        result = (result & state.textAndMask) ^ state.textXorMask;
        WriteChar(state.backposx, state.backposy, page, (uint8_t) (result & 0xff), (uint8_t) (result >> 8), true);
    } else {
...
//    if (real_readb(BIOSMEM_SEG,BIOSMEM_CURRENT_PAGE) != state.page) return;

// Check if cursor in update region
/*    if ((GETPOS_X >= state.updateRegion_x[0]) && (GETPOS_X <= state.updateRegion_x[1]) &&
        (GETPOS_Y >= state.updateRegion_y[0]) && (GETPOS_Y <= state.updateRegion_y[1])) {
   ...
   */ /*Not sure yet what to do update region should be set to ??? */
    ...
    uint16_t addx1, addx2, addy;
    uint16_t dataPos   = 0;
    int16_t  x1        = GETPOS_X / xratio - state.hotx;
    int16_t  y1        = GETPOS_Y - state.hoty;
    int16_t  x2        = x1 + X_CURSOR - 1;
    int16_t  y2        = y1 + Y_CURSOR - 1;    
    ...
    state.background = true;
    state.backposx   = GETPOS_X / xratio - state.hotx;
    state.backposy   = GETPOS_Y - state.hoty;
    ...
    for (y = y1; y <= y2; y++) {
        ...
        for (x = x1; x <= x2; x++) {
            uint8_t pixel = 0;
            // ScreenMask
            if (scMask & HIGHESTBIT) pixel = state.backData[dataPos];
            scMask<<=1;
            // CursorMask
...
static void SetMickeyPixelRate(const int16_t px, const int16_t py)
{
    if ((px != 0) && (py != 0)) {
        ...
        state.pixelPerMickey_x   = X_MICKEY / static_cast<float>(px);
        state.pixelPerMickey_y   = Y_MICKEY / static_cast<float>(py);    
    }
}
...
void MOUSEDOSDRV_AfterNewVideoMode(const bool setmode)
{
    ...
    case 0x0e:
    case 0x13:
        if (mode == 0x0d || mode == 0x13) state.gran_x = (int16_t) 0xfffe;
        state.max_y = 199;
        break;
    case 0x0f:
    case 0x10:
    ...
    state.language             = 0;
    state.page                 = 0;
    state.doubleSpeedThreshold = 64;
    state.updateRegion_y[1]    = -1; // offscreen
    state.cursorType           = 0;
    state.enabled              = true;
...
bool MOUSEDOSDRV_NotifyMoved(const int16_t x_rel, const int16_t y_rel, const uint16_t x_abs, const uint16_t y_abs, const bool is_captured)
{
    ...
    float dx = x_rel_sens * state.pixelPerMickey_x;
    float dy = y_rel_sens * state.pixelPerMickey_y;

    if((fabs(x_rel_sens) > 1.0) || (state.senv_x < 1.0)) dx *= state.senv_x;
    if((fabs(y_rel_sens) > 1.0) || (state.senv_y < 1.0)) dy *= state.senv_y;

    state.mickey_x += (dx * state.mickeysPerPixel_x);
    state.mickey_y += (dy * state.mickeysPerPixel_y);
    if (state.mickey_x >= 32768.0f) state.mickey_x -= 65536.0f;
    else if (state.mickey_x <= -32769.0f) state.mickey_x += 65536.0f;
    if (state.mickey_y >= 32768.0f) state.mickey_y -= 65536.0f;
    else if (state.mickey_y <= -32769.0f) state.mickey_y += 65536.0f;
    if (is_captured) {
        state.x += dx;
        state.y += dy;
    } else {
        ...
        if (CurMode->type == M_TEXT) {
            ...
            state.y = y * (IS_EGAVGA_ARCH ? (real_readb(BIOSMEM_SEG, BIOSMEM_NB_ROWS) + 1) : 25) * 8;
        } else if ((state.max_x < 2048) || (state.max_y < 2048) || (state.max_x != state.max_y)) {
    ...
    if (state.x > state.max_x) state.x = state.max_x;
    if (state.x < state.min_x) state.x = state.min_x;
    if (state.y > state.max_y) state.y = state.max_y;
...
static Bitu INT33_Handler()
{
    ...
    case 0x04: // MS MOUSE v1.0+ - position mouse cursor
        ...
        if ((int16_t) reg_cx >= state.max_x) state.x = static_cast<float>(state.max_x);
        else if (state.min_x >= (int16_t) reg_cx) state.x = static_cast<float>(state.min_x); 
        else if ((int16_t) reg_cx != GETPOS_X) state.x = static_cast<float>(reg_cx);

        if ((int16_t) reg_dx >= state.max_y) state.y = static_cast<float>(state.max_y);
        else if (state.min_y >= (int16_t) reg_dx) state.y = static_cast<float>(state.min_y); 
        else if ((int16_t) reg_dx != GETPOS_Y) state.y = static_cast<float>(reg_dx);
        MOUSEDOSDRV_DrawCursor();
        break;
    case 0x05: // MS MOUSE v1.0+ / CuteMouse - return button press data / mouse wheel data
        {
            uint16_t idx = reg_bx; // button index
            if (idx == 0xffff && state.cute_mouse) { // 'magic' index for checking wheel instead of button
                reg_bx = GetResetWheel16bit();
                reg_cx = state.last_wheel_moved_x;
                reg_dx = state.last_wheel_moved_y;
            } else {
                reg_ax = state.buttons;
                if (idx >= NUM_BUTTONS)
                    idx = NUM_BUTTONS - 1;
    ...
    case 0x0a: // MS MOUSE v3.0+ - define text cursor
        state.cursorType  = (reg_bx ? 1 : 0);
        state.textAndMask = reg_cx;
        state.textXorMask = reg_dx;
        if (reg_bx) {
    ...
    case 0x27: // MS MOUSE v7.01+ - get screen/cursor masks and mickey counts
        reg_ax = state.textAndMask;
        reg_bx = state.textXorMask;
        [[fallthrough]];
    ...
    case 0x10: // MS MOUSE v1.0+ - define screen region for updating
        state.updateRegion_x[0] = (int16_t) reg_cx;
        state.updateRegion_y[0] = (int16_t) reg_dx;
        state.updateRegion_x[1] = (int16_t) reg_si;
        state.updateRegion_y[1] = (int16_t) reg_di;
        MOUSEDOSDRV_DrawCursor();
        break;
    ...
    case 0x13: // MS MOUSE v5.0+ - set double-speed threshold
        state.doubleSpeedThreshold = (reg_bx ? reg_bx : 64);
        break;
    case 0x14: // MS MOUSE v3.0+ - exchange event-handler
        {
            uint16_t oldSeg  = state.sub_seg;
            uint16_t oldOfs  = state.sub_ofs;
            uint16_t oldMask = state.sub_mask;
            // Set new values
    ...
    case 0x1f: // MS MOUSE v6.0+ - disable mouse driver
        ...
        reg_bx = 0;
        SegSet16(es, 0);       
        state.enabled   = false; // Just for reporting not doing a thing with it
    ...
    case 0x2a: // MS MOUSE v7.01+ - get cursor hot spot
        reg_al = (uint8_t) -state.hidden; // Microsoft uses a negative byte counter for cursor visibility
        reg_bx = (uint16_t) state.hotx;
        reg_cx = (uint16_t) state.hoty;
