...
// XXX TODO
// XXX MOUSEDOSDRV_HasCallback() - to be removed; there shouldn't be a call, but
// notification to mouse.cpp
...
enum class EventType : uint8_t { // compatible with DOS driver mask in driver
	                         // function 0x0c
	NotDosEvent    = 0,
	MouseHasMoved  = 1 << 0,
	PressedLeft    = 1 << 1,
...
typedef struct MouseEvent {
	bool req_ps2 = false; // true = PS/2 mouse emulation needs an IRQ12 /
	                      // INT74 event
	...
	MouseEvent() {}
	MouseEvent(EventType dos_type)
	        : dos_type(static_cast<uint8_t>(dos_type))
	{}
...
static uint8_t buttons_12 = 0;  // state of buttons 1 (left), 2 (right), as
                                // visible on host side
static uint8_t buttons_345 = 0; // state of mouse buttons 3 (middle), 4, and 5
                                // as visible on host side
...
static void AddEvent(MouseEvent &event)
{
	...
	if (queue_used < QUEUE_SIZE) {
		...
		if (queue_used) {
			...
			if (event.dos_type ==
			            static_cast<uint8_t>(EventType::MouseHasMoved) ||
			    event.dos_type ==
			            static_cast<uint8_t>(EventType::WheelHasMoved)) {
				return;
			}
		...
		queue[0]             = event;
		queue[0].dos_buttons = static_cast<uint8_t>(
		        buttons_12 + (buttons_345 ? 4 : 0));
		queue_used++;
	}
...
static uintptr_t INT74_Handler()
{
	...
	if (queue_used) {
		...
		// INT 33h emulation: HERE within the IRQ 12 handler is the
		...
		if (MOUSEDOSDRV_HasCallback()) {
			...
			CPU_Push16(RealOff(CALLBACK_RealPointer(int74_ret_callback)) +
			           7);
			return MOUSEDOSDRV_DoCallback(event.dos_type,
			                              event.dos_buttons);
		}
	}
...
{
	const uint8_t buttons_12S_old = static_cast<uint8_t>(
	        buttons_12 + (buttons_345 ? 4 : 0));
	...
	const uint8_t buttons_12S = static_cast<uint8_t>(buttons_12 +
	                                                 (buttons_345 ? 4 : 0));
	...
	event.req_ps2 = MOUSEPS2AUX_NotifyPressedReleased(buttons_12S,
	                                                  buttons_12 | buttons_345);
	if (changed_12S) {
		event.req_ps2 = MOUSEVMWARE_NotifyPressedReleased(buttons_12S) ||
		                event.req_ps2;
