...
void MOUSECTL::WriteInterfaceName(const MouseInterfaceId interface_id)
{
    switch (interface_id) {
    case MouseInterfaceId::DOS:
        WriteOut(convert_ansi_markup("[color=cyan]DOS [reset]").c_str());
        break;
    case MouseInterfaceId::PS2:
        WriteOut(convert_ansi_markup("[color=cyan]PS/2[reset]").c_str());
        break;
    case MouseInterfaceId::COM1:
        WriteOut(convert_ansi_markup("[color=cyan]COM1[reset]").c_str());
        break;
    case MouseInterfaceId::COM2:
        WriteOut(convert_ansi_markup("[color=cyan]COM2[reset]").c_str());
        break;
    case MouseInterfaceId::COM3:
        WriteOut(convert_ansi_markup("[color=cyan]COM3[reset]").c_str());
        break;
    ...
    case MouseInterfaceId::None:
        ...
        break;
    default:
        LOG_ERR("MOUSECTL::WriteInterfaceName - unimplemented");
        break;
    }
}

void MOUSECTL::WriteMapStatus(const MouseMapStatus map_status)
{
    switch (map_status) {
    case MouseMapStatus::HostPointer:
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_STATUS_HOST"));
        break;
    case MouseMapStatus::Mapped:
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_STATUS_MAPPED"));
        break;
    case MouseMapStatus::Disconnected:
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_STATUS_DISCONNECTED"));
        break;
    case MouseMapStatus::Disabled:
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_STATUS_DISABLED"));
        break;
    default:
        LOG_ERR("MOUSECTL::WriteMapStatus - unimplemented");
        break;
    }
}
...
bool MOUSECTL::CmdShow(const bool show_all)
{
    ...
    for (const auto &entry : info_interfaces) {
        ...
        if (interface_id == MouseInterfaceId::COM1 ||
            ...
            hint_rate_com = true;
        WriteInterfaceName(interface_id);
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_INTERFACE_SPACING"));
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_SENSITIVITY"),
                 entry.GetSensitivityX(), entry.GetSensitivityY());
        ...
        if (entry.GetMinRate()) {
            ...
            WriteOut("  ");
        if (rate_hz)
            WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_RATE_HZ"), rate_hz);
        else
            WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_RATE_NA"));
        WriteMapStatus(entry.GetMapStatus());
        WriteOut("\n");
    ...
    for (const auto &entry : info_interfaces) {
        if (!entry.IsMapped() || entry.IsMappedDeviceDisconnected())
            continue;
        WriteInterfaceName(entry.GetInterfaceId());
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_INTERFACE_SPACING"));
        WriteOut(entry.GetMappedDeviceName().c_str());
        WriteOut("\n");
    }
    ...
    for (const auto &entry : info_physical) {
        if (entry.IsMapped() || entry.IsDeviceDisconnected())
            continue;
        WriteOut(MSG_Get("SHELL_CMD_MOUSECTL_TABLE_INTERFACE_UNMAPPED"));
        WriteOut(entry.GetDeviceName().c_str());
        WriteOut("\n");
    }
    WriteOut("\n");
